...
class BelgianGPPredictor:
    ...
    def fetch_weather_forecast(self, lat=50.4372, lon=5.9714):
        """Fetch weather forecast for Spa-Francorchamps"""
        print("🌤️ Fetching weather forecast for Belgian GP...")
        ...
        try:
            ...
            # Find forecast closest to July 27, 2025, 14:00
            forecast = next((item for item in data['list'] if '2025-07-27 14:00' in item['dt_txt']), data['list'][0])
    ...
    def prepare_race_data(self):
        """Prepare 2025 Belgian GP data based on 2021-2025 performance"""
        print("🏁 Preparing 2025 Belgian GP data...")
        
        # Filter 2025 season data
        season_2025_data = self.historical_data[self.historical_data['season'] == 2025].copy()
        
        # Get historical Spa performance (2021-2024)
        spa_data = self.historical_data[
            (self.historical_data['circuit_id'] == 'spa') & 
            (self.historical_data['season'].between(2016, 2024))
        ].copy()
        
        if season_2025_data.empty:
            print("❌ No 2025 data found in CSV. Using Spa historical data.")
            driver_performance = spa_data.groupby('driver_id').agg({
                'grid': 'mean',
                'position': 'mean',
                'points': 'sum',
                'constructor_id': 'last',
            ...
            # Incorporate Spa-specific performance (2021-2024)
            spa_performance = spa_data.groupby('driver_id').agg({
                'grid': 'mean',
                'position': 'mean',
                'points': 'sum'
            }).reset_index().rename(columns={
                'grid': 'spa_grid_avg',
                'position': 'spa_position_avg',
                'points': 'spa_points_sum'
            })
            
            driver_performance = driver_performance.merge(spa_performance, on='driver_id', how='left')
            driver_performance['spa_grid_avg'] = driver_performance['spa_grid_avg'].fillna(10.5)
            driver_performance['spa_position_avg'] = driver_performance['spa_position_avg'].fillna(10.5)
            driver_performance['spa_points_sum'] = driver_performance['spa_points_sum'].fillna(0)
            
            # Combine season and Spa performance for grid ranking
            driver_performance['combined_score'] = (
                0.7 * (driver_performance['points'] / (driver_performance['position'] + 1)) +
                0.3 * (driver_performance['spa_points_sum'] / (driver_performance['spa_position_avg'] + 1))
            )
        
        # Sort drivers by performance to estimate Belgian GP grid
        ...
        race_data = {
            ...
            'time_millis': [5100000] * len(drivers),
            'laps': [44] * len(drivers)
        }
    ...
    def predict_belgian_gp(self):
        """Predict the 2025 Belgian GP results"""
        print("🔮 Predicting 2025 Belgian GP results...")
    ...
    def display_predictions(self, results_df):
        ...
        print("\n" + "="*80)
        print("🏆 2025 BELGIAN GRAND PRIX - RACE PREDICTIONS")
        print("="*80)
    ...
    def create_visualizations(self, results_df):
        ...
        ax1.set_title('Grid vs Predicted Position (Belgian GP)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # Win Probabilities
        top_10 = results_df.head(10)
        ax2.bar(range(len(top_10)), top_10['win_probability']*100, color='gold', alpha=0.8)
        ax2.set_xlabel('Driver')
        ax2.set_ylabel('Win Probability (%)')
        ax2.set_title('Top 10 Win Probabilities')
        ax2.set_xticks(range(len(top_10)))
        ax2.set_xticklabels(top_10['driver_id'], rotation=45)
        ax2.grid(True, alpha=0.3)
        
        # Podium Probabilities
        ax3.bar(range(len(top_10)), top_10['podium_probability']*100, color='orange', alpha=0.8)
        ax3.set_xlabel('Driver')
        ax3.set_ylabel('Podium Probability (%)')
        ax3.set_title('Top 10 Podium Probabilities')
        ...
        ax4.set_title('Constructor Points Prediction')
        ax4.tick_params(axis='x', rotation=45)
        ax4.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig('belgian_gp_2025_predictions.png', dpi=300, bbox_inches='tight')
        plt.show()
        print("✅ Visualizations saved as 'belgian_gp_2025_predictions.png'")
    ...
    def save_predictions(self, results_df):
        ...
        output_df.to_csv('belgian_gp_2025_predictions.csv', index=False)
        print("✅ Predictions saved to 'belgian_gp_2025_predictions.csv'")
...
if __name__ == "__main__":
    ...
    if predictor.load_model_and_data():
        ...
        print("\n🎉 Belgian GP 2025 predictions complete!")
    else:
